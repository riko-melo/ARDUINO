...
void dht11(OLEDDisplay * display, OLEDDisplayUiState * state, int16_t x, int16_t y) {
  ...
  if (isnan(umidade) || isnan(celsius) || isnan(fahrenheit) ) {
    ...
    display->drawString(relogio_X + x, relogio_Y + y, "Erro, verifique o sensor DHT!");
    display->display();
    delay(3000);
    display->clear();
    //return;
    ...
    display->drawString(relogio_X + x, relogio_Y + y, String(celsius) + "ºC");
    display->display();
    delay(5000);
    display->clear();
    ...
    display->drawString(0, 5, "TEMPERATURA: ");
    display->setFont(ArialMT_Plain_24);
    display->drawString(relogio_X + x, relogio_Y + y, String(fahrenheit) + "ºF");
    display->display();
    delay(5000);
    display->clear();
    ...
    display->setFont(ArialMT_Plain_24);
    display->drawString(relogio_X + x, relogio_Y + y, String(umidade) + "%");
    display->display();
    delay(5000);
    display->clear();
...
void calendario (OLEDDisplay * display, OLEDDisplayUiState * state, int16_t x, int16_t y) {
  ...
  display->setFont(ArialMT_Plain_16);
  display->drawString(relogio_X + x, relogio_Y + y, String (semana[7][12]));
  display->setFont(ArialMT_Plain_24);
  unsigned long novoTempo = ntp.getEpochTime();
  data = novoTempo - 2208988800UL;

  while ((unsigned)(dia += (LEAP_YEAR(ano) ? 366 : 365)) <= data) {
    ano++;
  }
  ano = ano; // *pyear is returned as years from 1900

  dia -= LEAP_YEAR(ano) ? 366 : 365;
  data -= dia; // now it is days in this year, starting at 0

  for (int meses = 0; meses < 12; meses++) {
    mesSeguinte = ( (meses == 1) && LEAP_YEAR(ano) ) ? 29 : monthDays[meses]; // month==1 -> february
    if (data >= mesSeguinte) {
      data -= mesSeguinte;
    } else {
      break;
    }
    //  display->drawString(relogio_X + x, relogio_Y + y, String (String(dia) + "/" String(meses) + "/" + String(ano));
    display->drawString(relogio_X + x, relogio_Y + y, String (data));
  }
}
...
void relogioDigital(OLEDDisplay * display, OLEDDisplayUiState * state, int16_t x, int16_t y) {
  ...
  for (int g = 0; g < 120; g++)
  {
    ...
    display->setTextAlignment(TEXT_ALIGN_CENTER);
    display->setFont(ArialMT_Plain_24);
    display->drawString(relogio_X + x, relogio_Y + y, agora);
...
void exibirSMS(OLEDDisplay * display, OLEDDisplayUiState * state, int16_t x, int16_t y) {
  ...
  display->drawString (relogio_X + x, relogio_Y + y, "MENSAGEM");
  display->drawString (relogio_X + x, relogio_Y + y, "URGENTE!");
  display->display();
  delay(3000);
  display->clear();
  ...
  display->setTextAlignment(TEXT_ALIGN_LEFT);
  display->setFont(ArialMT_Plain_10);
  display->drawString (relogio_X + x, relogio_Y + y, "DE");
  //34, 0
  display->setFont(ArialMT_Plain_16);
  display->drawString (relogio_X + x, relogio_Y + y, "HIGO");
  //23,13
  display->setFont(ArialMT_Plain_10);
  display->drawString (relogio_X + x, relogio_Y + y, "PARA");
  //30,32
  display->setFont(ArialMT_Plain_24);
  display->drawString (relogio_X + x, relogio_Y + y, "REBECA");
  //10,45
  display->display();
  delay(3000);
  display->clear();
  ...
  for (uint8_t i = 0; i < 2; i++)
  {
    for (int16_t j = -127; j < 192; j++)
    {
      ...
      {
        display->setFont(ArialMT_Plain_16);
        display->drawString(j - 164, 17 * i, "SAUDADES!!!");
        display->display();
        delay(3000);
        display->clear();
      }
...
FrameCallback frames[] = {calendario, relogioDigital, exibirSMS, dht11, buscarWLAN};
int frameContador = 5;
...
void medirWLAN()
{
  ...
  for (int h = 0; h < 30; h++)
  {
    ...
    display.drawProgressBar(0, 40, 128, 10, sinal);
    display.display();
    delay(150);
    display.clear();
  }
